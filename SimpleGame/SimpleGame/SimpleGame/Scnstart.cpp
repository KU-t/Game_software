...
void Scnstart::Update(float fTimeElapsed) {
	for (int i = 0; i < MAX_OBJECT; ++i)
	{
		if (m_Objects[i]) {
...
void Scnstart::ApplyForce(float x, float y, float z, DWORD eTime) {
	STATE state;
...
void Scnstart::AddObject(float pX, float pY, float pZ, float hight, float sX, float sY, float vX, float vY, float vZ, KIND kind, int hp, STATE state) {
...
void Scnstart::DeleteObject(unsigned int id) {
	if (m_Objects[id]) {
		delete m_Objects[id];
		m_Objects[id] = NULL;
	}
...
void Scnstart::Shoot(int shootID, DWORD fTimeElapsed) {
...
bool Scnstart::RRCollision(float minX, float maxX, float minY, float maxY, float minX1, float maxX1, float minY1, float maxY1) {
...
bool Scnstart::IsCollision(float minx, float miny, float maxx, float maxy, float minx1, float miny1, float maxx1, float maxy1) {
	if (maxx <= minx1)	return false;
	if (minx >= maxx1)	return false;
...
void Scnstart::processCollision(int i, int j) {
